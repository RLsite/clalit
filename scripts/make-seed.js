// Converts scrape/*.json (base44 export) into src/seed-data.js
...
// Device image files copied from the original site (images/devices/, repo root)
const deviceImages = {};
for (const id of homeOrder) {
  for (const ext of ['png', 'jpg']) {
    if (fs.existsSync(path.join(__dirname, '..', 'images', 'devices', `${id}.${ext}`))) {
...
fs.writeFileSync(path.join(__dirname, '..', 'src', 'seed-data.js'), out, 'utf8');
